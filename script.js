(function () {
  ...
  const visuals = Array.from(document.querySelectorAll(".product-visual"));
  if (!logo || !visuals.length) return;
  ...
  // strip at the top of the screen, so entries only fire while a grey
  // product-visual box is passing directly behind the fixed logo.
